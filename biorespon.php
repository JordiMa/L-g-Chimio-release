...
if ($row[0]=='{RESPONSABLE}' or $row[0]=='{CHEF}') {
	$equipebio="";
	if ($row[0]=='{CHEF}') {
		$sql="SELECT distinct(chi_id_equipe) FROM chimiste WHERE chi_id_responsable='$row[1]'";
		$resultchef=$dbh->query($sql);
		$numresultchef=$resultchef->rowCount();
		$y=0;
		while($rowchef=$resultchef->fetch(PDO::FETCH_NUM)) {
			$equipebio.="pro_id_equipe='".$rowchef[0]."'";
			$y++;
			if($y<$numresultchef) $equipebio.=" or ";
		}
	}
	elseif ($row[0]=='{RESPONSABLE}') $equipebio.="pro_id_responsable='".$row[1]."'";
	...
	if ($numresultat1>0) {
		...
		print"<tr><td>";
		$formulaire=new formulaire ("cible","resultatbiorespon.php","POST",true);
	...
	if (isset($_POST["cible"]) and !empty($_POST["cible"])) {
		$formulaire1=new formulaire ("cible1","resultatbiorespon.php","POST",true);
		...
		print"</td><td>";
		$sql="SELECT lab_id_labocible,lab_concentration,lab_protocol,lab_laboratoire FROM cible,labocible WHERE cib_id_cible='".$_POST["cible"]."' and cible.cib_id_cible=labocible.lab_id_cible";
		$resultat3=$dbh->query($sql);

		print "<label>".DESCRIPT."</label><div style=\"width:500; height:200; overflow:auto; border:solid 1px black;\">";
		$formulaire2=new formulaire ("cible2","resultatbiorespon.php","POST",true);
		...
		while($row3=$resultat3->fetch(PDO::FETCH_NUM)) {
			...
			print"<td><i>".LABO."</i><br/>$row3[3]<br/><i>".CONCEN."</i><br/>$row3[1]<br/><i>".PROTOCOL."</i><br/>$row3[2]</td></tr>";
		}
		print "</table>";
	...
	if (!empty($_POST["cible"]) and !empty($_POST["labo"]) and !empty($_POST["colactiv"])) {
		...
		//nombre limite de molécules à afficher
		$limitepage=10;
		...
		$sql="SELECT res_id_produit FROM resultat,produit WHERE res_id_labocible='".$_POST["labo"]."' and resultat.res_id_produit=produit.pro_id_produit and ($equipebio)";
		...
		if ($nbpage>1) {
			...
			for ($i=1; $i<=$nbpage; $i++) {
				...
				else print "<a href=\"resultatbiorespon.php?cible=".$_POST["cible"]."&labo=".$_POST["labo"]."&colactiv=".$_POST["colactiv"]."&ordre=".$_POST["ordre"]."&page=$i&champ=".$_POST["champ"]."\"><strong>$i</strong></a>";
				if ($i<$nbpage) print", ";
			}	
			print"</p>";
		}
		...
		if (!empty($champ) and !empty($_POST["ordre"]))	{
			if ($champ=="res_resultat_ic50" or $champ=="res_resultat_ec50" or $champ=="res_resultat_pourcentactivite" or $champ=="res_resultat_pourcentageinhi") $sql.=" ORDER BY CAST($champ AS unsigned) ".$_POST["ordre"]."";
			elseif ($champ=="res_actif") $sql.=" ORDER BY CAST($champ AS unsigned) ".$_POST["ordre"]."";
			elseif ($champ=="res_resultat_autre") $sql.=" ORDER BY $champ ".$_POST["ordre"]."";
		}
		...
		if ($_POST["colactiv"]<>1) {
			...
			$formulaire4=new formulaire ("cible1","resultatbiorespon.php","POST",true);
			...
			$formulaire4->fin();
			print"</th></tr></table></th><th scope=\"col\" width=\"40%\">".COMMENTAIRES."</th>";
			print"</th></tr>";
		}
		else {
			...
			$formulaire4->fin();
			print"</th><th>";
			$formulaire5=new formulaire ("cible1","resultatbiorespon.php","POST",true);
			...
			$formulaire6=new formulaire ("cible1","resultatbiorespon.php","POST",true);
			...
			$formulaire6->fin();
			print"</th><th>";
			$formulaire7=new formulaire ("cible1","resultatbiorespon.php","POST",true);
			...
			$formulaire8=new formulaire ("cible1","resultatbiorespon.php","POST",true);
			...
			$formulaire8->fin();
			print"</th><th>";
			$formulaire9=new formulaire ("cible1","resultatbiorespon.php","POST",true);
			...
			$formulaire10=new formulaire ("cible1","resultatbiorespon.php","POST",true);
			...
			$formulaire10->fin();
			print"</th><th>";
			$formulaire11=new formulaire ("cible1","resultatbiorespon.php","POST",true);
			...
			$formulaire12=new formulaire ("cible1","resultatbiorespon.php","POST",true);
			$formulaire12->affiche_formulaire();
			if (isset($_POST["ordre"]) and $_POST["ordre"]=="desc" and $_POST["champ"]==4) $formulaire12->ajout_buttonimage ("","download","image","","images/ascr.gif",DESC);
			...
			$formulaire12->ajout_cache (4,"champ");
			$formulaire12->ajout_cache ("desc","ordre");
			$formulaire12->fin();
			print"</th><th>";
			$formulaire13=new formulaire ("cible1","resultatbiorespon.php","POST",true);
			$formulaire13->affiche_formulaire();
			if (isset($_POST["ordre"]) and $_POST["ordre"]=="asc" and $_POST["champ"]==4) $formulaire13->ajout_buttonimage ("","download","image","","images/descr.gif",ASC);
			...
			$formulaire14->fin();
			print"</th><th>";
			$formulaire15=new formulaire ("cible1","resultatbiorespon.php","POST",true);
			...
			$formulaire15->fin();
			print"</th></tr></table></th><th scope=\"col\" width=\"50%\">".COMMENTAIRES."</th>";
			print"</th></tr>";
		}
				...
				while($row4=$resultat4->fetch(PDO::FETCH_NUM)) {
					if ($_POST["colactiv"]<>1) {
						...
						if ($_POST["modif"]==1 and $_POST["produit"]==$row2[0]) {
							$formulaire16=new formulaire ("cible1","resultatbiorespon.php","POST",true);
							$formulaire16->affiche_formulaire();
							print"<td>";
							$formulaire16->ajout_text (6, $row4[0], 6, "mcol","","","");
							print"</td><td width=\"50%\"><div style=\"width:100%; height:200; overflow:auto; border:solid 1px black;\">";
							...
							print"</td><td width=\"50%\"><div style=\"width:100%; height:200; overflow:auto; border:solid 1px black;\"><p align=\"left\">".nl2br($row4[1])."</p></div></td>";
						}
						print"</tr>";
					}
					else {
						...
						if ($_POST["modif"]==1 and $_POST["produit"]==$row2[0]) {
							...
							$formulaire18=new formulaire ("cible1","resultatbiorespon.php","POST",true);
							...
							print">&nbsp;</td><td>$row4[3]</td><td>$row4[4]</td><td>$row4[5]</td><td width=\"50%\"><div style=\"width:100%; height:200; overflow:auto; border:solid 1px black;\"><p align=\"left\">".nl2br($row4[6])."</p></div></td>";
							}
						print"</tr>";	
					}
					$com++;
