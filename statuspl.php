...
if (isset($_POST["labo"]) and !empty($_POST["labo"])) {
  $sql="INSERT INTO plaquecible (plac_id_cible,plac_id_plaque,plac_date) VALUES ('".$_POST["labo"]."','".$_POST["idpl"]."','".$_POST['annee']."-".$_POST["mois"]."-".$_POST["jour"]."')";
  $insert=$dbh->exec($sql);
}
if (isset($_POST['nomcible']) and !empty($_POST['nomcible']) and isset($_POST['iniprocible']) and isset($_POST['conccible']) and !empty($_POST['conccible']) and isset($_POST['protocible']) and !empty($_POST['protocible'])  and isset($_POST['labocible']) and !empty($_POST['labocible'])) {
  $_POST['conccible']=str_replace(",",".",$_POST['conccible']);
  $sql="INSERT INTO cible (cib_nom,cib_uniprot) VALUES ('".$_POST['nomcible']."','".$_POST['iniprocible']."')";
  $insert1=$dbh->exec($sql);
  $lastIdinsertion = $dbh->lastInsertId();
  $sql="INSERT INTO labocible (lab_concentration,lab_protocol,lab_laboratoire,lab_id_cible) VALUES ('".$_POST['conccible']."','".$_POST['protocible']."','".$_POST['labocible']."','$lastIdinsertion')";
  $insert2=$dbh->exec($sql);
  $lastIdinsertion1 = $dbh->lastInsertId();
  $sql="INSERT INTO plaquecible (plac_id_cible,plac_id_plaque,plac_date) VALUES ('$lastIdinsertion1','".$_POST["idpl"]."','".$_POST['annee']."-".$_POST["mois"]."-".$_POST["jour"]."')";
  $insert=$dbh->exec($sql);
  unset($_POST['nomcible']);
  unset($_POST['conccible']);
  unset($_POST['protocible']);
