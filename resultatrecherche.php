...
if ($_POST['masseexact']=='exacte' and $_POST['supinf']!="%3D")  {
	...
	if (!empty($_POST['mol']) or !empty ($_POST['formbrute']) or ($_POST['massemol'])!="" or !empty($_POST['numero']) or !empty($_POST['logp'])) {
		...
		if (!empty($_POST['numero'])) {
			$sql="SELECT DISTINCT pro_id_produit,pro_id_structure FROM produit,structure WHERE (pro_numero='".$_POST['numero']."' or pro_num_constant='".intval($_POST['numero'])."' or pro_num_cn='".$_POST['numero']."' or pro_qrcode='".$_POST['numero']."') and (pro_id_type<>2 or ($requetepartielle)) and structure.str_id_structure=produit.pro_id_structure order by pro_id_structure";
			$result4 = $dbh->query($sql);
			while ($row4=$result4->fetch(PDO::FETCH_NUM)) {
				$tab[$row4[0]]=$row4[1];
			}
