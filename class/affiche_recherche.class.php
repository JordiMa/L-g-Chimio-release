    ...
    if (empty ($this->resultatsql)) echo "<p align=\"center\"><br /><br /><strong>".RIEN."</strong></p>";
		...
		foreach($this->resultatsql as $key=>$element) {
				if ($element!=$idstruc) {
					...
					$sql= "SELECT chi_nom, chi_prenom,pro_date_entree,pro_numero,pro_masse,equi_nom_equipe,pro_unite_masse FROM produit,chimiste,equipe WHERE pro_id_produit=$key and produit.pro_id_chimiste=chimiste.chi_id_chimiste and pro_id_equipe=equi_id_equipe";
					...
					echo "<tr><td valign=\"top\">".$ror[3]." : ".$ror[4].constant($ror[6])."<br/>".$ror[0]." ".$ror[1].",&nbsp;".$date."<br/>".EQUI." ".$ror2[1]." ".$ror2[0]."</td><td align=\"right\" valign=\"middle\"><form method=\"post\" action=\"fiche.php\">
